...
test_installer_has_namespaced_module_boundary() {
    ...
    for module in core plan deploy platform packages postinstall main; do
        [[ -f "$PWD/installer/$module.sh" ]] || result=1
    done
...
test_home_mirror_defines_the_deployment_file_list() {
    local file_list
    local result=0

    file_list="$(mktemp)"
    bash -c 'source "$1"; ybw::deploy::list_tree_files home home "$2"' _ "$PWD/bootstrap.sh" "$file_list" > /dev/null || result=1
    [[ -s "$file_list" ]] || result=1
    grep -qx '.bashrc' "$file_list" || result=1
    ! grep -q '^home/' "$file_list" || result=1
    rm -f "$file_list"

    return "$result"
}

test_agent_skills_live_in_the_home_mirror() {
    local file_list
    local result=0
    local status=0

    [[ -L "$PWD/.agents" ]] || result=1
    [[ "$(readlink "$PWD/.agents")" == "home/.agents" ]] || result=1
    [[ -f "$PWD/.agents/skills/ship/SKILL.md" ]] || result=1

    file_list="$(mktemp)"
    bash -c 'source "$1"; ybw::deploy::list_tree_files home home "$2"' _ "$PWD/bootstrap.sh" "$file_list" > /dev/null || result=1
    grep -qx '.agents/AGENTS.md' "$file_list" || result=1

    bash -c 'source "$1"; ybw::deploy::list_tree_files missing-install-root "" "$2"' _ "$PWD/bootstrap.sh" "$file_list" > /dev/null 2>&1 || status=$?
    [[ $status -ne 0 ]] || result=1
    rm -f "$file_list"
