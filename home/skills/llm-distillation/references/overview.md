...
> Historical taxonomy and research notes, not a claim that every listed method is current or independently verified. Confirm a paper and its setup before relying on comparative results.
...
- **`foundations.md`** — Temperature scaling, divergences, token-level distillation, teacher-student formulation, implementation notes
- **`offline-online.md`** — Offline distillation, online distillation (Deep Mutual Learning), semi-online hybrids
- **`off-policy.md`** — Off-policy distillation, sequence-level KD, logit distillation, synthetic data pipelines
- **`on-policy.md`** — GKD, OPD, REOPOLD, ExOPD, RL–distillation hybrids (SDPO, RLSD, SRPO, OpenClaw-RL), failure modes
- **`self-distillation.md`** — Self-distillation forms, OPSD, RL via self-distillation, hindsight alignment, agentic SD
- **`multi-teacher.md`** — MOPD, see-saw problem, teacher routing, Nemotron-Cascade 2
- **`decision-guide.md`** — When to use each method, comparison tables, practical progressions
