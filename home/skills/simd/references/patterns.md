...
> 原有 Lemire 相关文章与代码笔记保留在此；样例数据不代表当前目标性能，省略号片段不是可直接提交的生产代码。使用前核对原始来源、ISA、边界条件并运行基准。
...
| 平台 | 推荐指令集 | 寄存器宽度 | 特色指令 |
|------|----------|----------|---------|
| Intel/AMD（支持相应扩展时） | AVX-512 及具体子扩展需分别检测 | 512 bit (64 byte) | IFMA、VBMI 等不能仅凭 AVX-512 总称推定 |
| Intel/AMD（消费者） | AVX2 | 256 bit (32 byte) | VPSHUFB、VPMULLD |
| ARM 64-bit（通用） | NEON | 128 bit (16 byte) | VLD4、VTBL/VQTBL |
| ARM（具体型号需核对） | 仅使用目标实际支持的 SVE/SVE2 | 查询实际向量长度 | SVE 与 SVE2 不能互相等同 |
| Apple Silicon | NEON（无 SVE2！） | 128 bit | NEON 完整支持 |

**重要**：以具体 CPU、OS 保存状态能力和编译目标验证 ISA；不要把品牌、年份或某一代型号推断为全部设备能力。
...
3. **Shuffle 控制位**：128-bit `pshufb` 的 bit 7 为 1 时将对应结果字节清零；否则用低 4 位选择该 128-bit lane 内的源字节。不是索引 ≥ 16 就清零。
...
PSHUFB 控制位修订依据：[Clang intrinsic 文档](https://clang.llvm.org/doxygen/tmmintrin_8h.html)。
