...
phase_package_management() {
    ...
    local activate_home_manager="$5"
    local home_manager_profile="$6"
    ...
    if [[ "$install_toolchain" == true ]]; then
        if ! install_mise_toolchains; then
            log_error "Toolchain installation failed"
            exit 1
        fi
    else
        log_info "Skipping Mise language toolchains (use --toolchain to install)"
    fi
...
install_npx_tools() {
    if ! check_command mise; then
        log_warn "Mise is not installed. Skipping npx tools installation."
        return 0
    fi
...
install_mise_toolchains() {
    ...
    if ! check_command mise; then
        log_error "Mise is required to install language toolchains"
        log_info "Activate the Home Manager configuration, then rerun with --toolchain"
        return 1
    fi

    log_info "Installing language toolchains with Mise..."
    if ! (cd "$HOME" && mise install); then
        log_error "Mise failed to install the configured language toolchains"
        return 1
    fi

    install_go_tools
    log_success "Mise language toolchains installed"
}

install_go_tools() {
    if ! check_command mise; then
        log_warn "Mise is not installed. Skipping Go tools installation."
        return 0
    fi
...
main() {
    ...
    if [[ "$dry_run" == false ]]; then
        phase_platform_setup
        phase_package_management "$install_mac_apps" "$install_npx" "$install_pi" "$install_toolchain" "$activate_home_manager" "$home_manager_profile"
        phase_postinstall
    else
