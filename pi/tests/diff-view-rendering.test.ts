...
} from "../extensions/diff-view/renderer/tool/diff/diff-renderer.ts";
...
} from "../extensions/diff-view/renderer/tool/diff/write-execution.ts";
...
test("rich diff handles only successful edit/write results", () => {
	const store = new WriteExecutionMetadataStore();
	assert.equal(renderRichToolResult("read", {}, {}, theme, {}, store), undefined);
	assert.equal(
		renderRichToolResult("edit", {}, { isPartial: true }, theme, {}, store),
		undefined,
	);
	assert.equal(renderRichToolResult("edit", {}, { isError: true }, theme, {}, store), undefined);
});
...
test("write execution captures the 512000-byte boundary and degrades above it", async () => {
	const directory = await mkdtemp(join(tmpdir(), "diff-view-"));
...
test("diff-view registers only edit and write tool overrides", () => {
	...
	assert.ok(registered.every((tool) => typeof tool.renderResult === "function"));
	...
	assert.match(output(rich).join("\n"), /old/);
	assert.match(output(rich).join("\n"), /new/);

	const nativeError = edit.renderResult(
		{ content: [{ type: "text", text: "edit failed" }] },
		{ isError: true },
		theme,
		{ args: { path: "sample.ts" }, state: {} },
	);
	assert.ok(nativeError, "failed edits fall back to Pi's native result renderer");
});

test("external write ownership is preserved", () => {
	const registered: any[] = [];
	installDiffViewTools({
		getAllTools() {
			return [
				{ name: "edit", sourceInfo: { source: "builtin" } },
				{ name: "write", sourceInfo: { source: "extension", path: "other.ts" } },
			];
		},
		registerTool(tool: unknown) {
			registered.push(tool);
	...
	assert.deepEqual(registered.map((tool) => tool.name), ["edit"]);
});
