...
ADRs created with this toolkit are **executable specifications for coding agents**. A human approves the decision; an agent implements it. The ADR must contain everything the agent needs to write correct code without asking follow-up questions.
...
Ask: **Does this capture your intent? Anything to add or correct?**

Do NOT proceed to Phase 2 until the human confirms the summary.

### Phase 2: Draft the ADR

1. **Use the ADR directory.** `/init-adr` initializes `docs/adr/`; store new records there. If historical ADRs exist elsewhere, read them during Phase 0 and either preserve their established convention or migrate them explicitly before drafting.
...
3. **Choose a template.**
   - Use `toolkit/assets/templates/adr-simple.md` for straightforward decisions (one clear winner, minimal tradeoffs).
   - Use `toolkit/assets/templates/adr-madr.md` when you need to document multiple options with structured pros/cons/drivers.
   - See `toolkit/references/template-variants.md` for guidance.
...
7. **Generate the file.**
   - Preferred: run `node docs/adr/toolkit/scripts/new_adr.js` (handles directory, naming, and optional index updates).
   - If you can't run scripts, copy a template from `docs/adr/toolkit/assets/templates/` and fill it manually.

### Phase 3: Review Against Checklist

After drafting, review the ADR against the agent-readiness checklist in `toolkit/references/review-checklist.md`.
...
- {specific gap 1, e.g. "Implementation Plan doesn't mention test files; identify the test suite that should cover this"}
- {specific gap 2}
...
// See: docs/adr/2025-06-15-use-sqlite-for-test-database.md
...
2. Use `node docs/adr/toolkit/scripts/set_adr_status.js` for status changes (supports YAML front matter, bullet status, and section status).
...
2. **Reference the ADR in PRs.** Link to the ADR in PR descriptions, e.g. "Implements `docs/adr/2025-06-15-use-sqlite-for-test-database.md`."
...
Preferred: let `node docs/adr/toolkit/scripts/new_adr.js --update-index` do it. Otherwise:
...
```bash
node docs/adr/toolkit/scripts/bootstrap_adr.js --dir docs/adr
```
...
- `toolkit/scripts/new_adr.js` — create a new ADR file from a template, using repo conventions.
- `toolkit/scripts/set_adr_status.js` — update an ADR status in-place (YAML front matter or inline). Use `--json` for machine output.
- `toolkit/scripts/bootstrap_adr.js` — create the ADR directory, `README.md`, and initial "Adopt ADRs" decision.

### `toolkit/references/`

- `toolkit/references/review-checklist.md` — agent-readiness checklist for Phase 3 review.
- `toolkit/references/adr-conventions.md` — directory, filename, status, and lifecycle conventions.
- `toolkit/references/template-variants.md` — when to use simple vs MADR-style templates.
- `toolkit/references/examples.md` — filled-out short and long ADR examples with implementation plans.

### `toolkit/assets/`

- `toolkit/assets/templates/adr-simple.md` — lean template for straightforward decisions.
- `toolkit/assets/templates/adr-madr.md` — MADR 4.0 template for decisions with multiple options and structured tradeoffs.
- `toolkit/assets/templates/adr-readme.md` — ADR index scaffold used by the bootstrap script.
...
```bash
# Simple ADR
node docs/adr/toolkit/scripts/new_adr.js --title "Choose database" --status proposed --update-index

# MADR-style with options
node docs/adr/toolkit/scripts/new_adr.js --title "Choose database" --template madr --status proposed --update-index

# Change status
node docs/adr/toolkit/scripts/set_adr_status.js docs/adr/2025-06-15-choose-database.md --status accepted

# Bootstrap the first adoption decision
node docs/adr/toolkit/scripts/bootstrap_adr.js --dir docs/adr
```
