...
`/init-adr` initializes `docs/adr/`; use it for new records.

If the repo already has historical ADRs elsewhere, read them and preserve their conventions unless an explicit migration moves them into `docs/adr/`. The toolkit scripts detect historical directories in this order: `contributing/decisions/`, `docs/decisions/`, `adr/`, `docs/adr/`, `docs/adrs/`, `decisions/`. Pass `--dir docs/adr` when the command must target the initialized directory.
