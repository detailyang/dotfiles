...
ybw::plan::validate() {
    ...
    if [[ "$YBW_INSTALL_PLAN_HOME_MANAGER" == true ]]; then
        if ! ybw::nix::load_profile; then
            ...
            ybw::log::info "Run ./scripts/install-nix.sh first"
            return 1
        fi
        if [[ ! -f "$YBW_INSTALL_ROOT/.config/home-manager/flake.nix" || ! -f "$YBW_INSTALL_ROOT/.config/home-manager/flake.lock" ]]; then
