...
check "Home Manager activation is explicit" "postinstall=\$(sed -n '/^phase_postinstall()/,/^}/p' install.sh); grep -Fq 'if [[ \"\$activate_home_manager\" == true ]]' <<< \"\$postinstall\" && grep -Fq 'setup_home_manager \"\$home_manager_profile\"' <<< \"\$postinstall\""
check "installer defaults to skipping Home Manager activation" "grep -Fq 'local activate_home_manager=false' install.sh && grep -Fq 'phase_postinstall \"\$activate_home_manager\" \"\$home_manager_profile\"' install.sh"
