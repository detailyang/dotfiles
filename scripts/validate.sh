...
check "Homebrew provides only the Fish executable" "fish_packages=\$(sed -n '/^readonly YBW_BREW_FISH_PACKAGES=(/,/^)/p' scripts/install/platform.sh); [[ \$(grep -c '^    \"' <<< \"\$fish_packages\") -eq 1 ]] && grep -Fqx '    \"fish\"' <<< \"\$fish_packages\""
...
check "standard macOS install provisions the Fish executable" "sed -n '/^ybw::packages::install_selected()/,/^}/p' scripts/install/packages.sh | grep -Fq 'if ! ybw::macos::install_fish; then'"
...
check "Homebrew package checks verify formula ownership" "grep -Fq 'brew list --formula \"\$package\"' scripts/install/platform.sh"
check "installer selects Homebrew Fish as login shell" "grep -Fq 'fish_path=\"\$(brew --prefix)/bin/fish\"' scripts/install/platform.sh && grep -Fq 'sudo chsh -s \"\$fish_path\" \"\$USER\"' scripts/install/platform.sh"
