...
check "installer provisions language toolchains through Mise" "grep -Fq 'ybw::packages::install_toolchains()' scripts/install/packages.sh && grep -Fq 'mise install' scripts/install/packages.sh && ! grep -Eq '^ybw::packages::install_official_(go|node|rust|toolchains)\\(\\)' scripts/install/packages.sh"
check "installer keeps Mise toolchains scoped to macOS" "block=\$(sed -n '/^ybw::packages::install_toolchains()/,/^}/p' scripts/install/packages.sh); grep -Fq 'if ! ybw::platform::is_macos; then' <<< \"\$block\""
check "installer activates Home Manager before Mise toolchains and npx tools" "block=\$(sed -n '/^ybw::packages::install_selected()/,/^}/p' scripts/install/packages.sh); home_manager_line=\$(grep -n 'ybw::packages::activate_home_manager' <<< \"\$block\" | cut -d: -f1); mise_line=\$(grep -n 'ybw::packages::install_toolchains' <<< \"\$block\" | cut -d: -f1); npx_line=\$(grep -n 'ybw::packages::install_npx' <<< \"\$block\" | cut -d: -f1); [[ -n \"\$home_manager_line\" && -n \"\$mise_line\" && -n \"\$npx_line\" && \$home_manager_line -lt \$mise_line && \$mise_line -lt \$npx_line ]]"
check "npx tools execute inside the Mise environment" "block=\$(sed -n '/^ybw::packages::install_npx()/,/^}/p' scripts/install/packages.sh); grep -Fq 'mise exec -- npx' <<< \"\$block\" && ! grep -Fq '/usr/bin/env npx' <<< \"\$block\" && ! grep -Fq 'ybw::command::exists npx' <<< \"\$block\""
check "legacy language version managers are disabled" "! grep -Fqx '    \"nvm\"' scripts/install/packages.sh && ! grep -REn '/usr/local/go/bin|\$HOME/(node|python)/bin|~/((node|python)/bin)' bash/.path fish/path.fish"
check "Darwin Home Manager provides Fish without changing Linux login-shell ownership" "grep -Fq 'pkgs.fish' .config/home-manager/apps/darwin-cli.nix && grep -Fq '../../apps/darwin-cli.nix' .config/home-manager/modules/platform/darwin.nix && ! grep -REn 'programs\\.fish|pkgs\\.fish' .config/home-manager/modules/common.nix .config/home-manager/modules/platform/linux.nix"
check "Homebrew is no longer used for formula packages" "! grep -Eq 'brew (install|list) --formula|brew install --build-from-source|YBW_BREW_(FISH|CLI)_PACKAGES' scripts/install/platform.sh"
check "retired macOS utilities remain removed" "! test -e .config/home-manager/apps/proxychains.nix && ! test -e fish/proxychains4.fish && ! grep -Eq 'pname = \"brightness\"|pkgs\\.(monitorcontrol|proxychains-ng)' .config/home-manager/apps/darwin-cli.nix .config/home-manager/home.nix && grep -Fq 'pname = \"im-select\"' .config/home-manager/apps/darwin-cli.nix"
...
check "Home Manager installs Jujutsu instead of tidwall jj" "grep -Fq 'pkgs.jujutsu' .config/home-manager/modules/common.nix && ! grep -Eq '^[[:space:]]*pkgs\.jj[[:space:]]*$' .config/home-manager/modules/common.nix"
...
check "standard macOS install resolves Fish independently of Homebrew" "grep -Fq 'ybw::macos::find_fish()' scripts/install/platform.sh && sed -n '/^ybw::packages::install_selected()/,/^}/p' scripts/install/packages.sh | grep -Fq 'fish_path=\"\$(ybw::macos::find_fish)\"'"
check "standard Linux install provisions the Fish executable" "sed -n '/^ybw::packages::install_selected()/,/^}/p' scripts/install/packages.sh | grep -Fq 'if ! ybw::linux::install_fish; then'"
check "Linux Fish installation supports common package managers" "for manager in apt-get dnf yum pacman zypper apk; do grep -Fq \"ybw::command::exists \$manager\" scripts/install/platform.sh || exit 1; done"
check "Arch Fish installation avoids a partial system upgrade" "grep -Fq 'ybw::command::as_root pacman -S --needed --noconfirm fish' scripts/install/platform.sh && ! grep -Fq 'pacman -Sy' scripts/install/platform.sh"
check "Homebrew is required only for remaining macOS casks" "block=\$(sed -n '/if \[\[ \"\$platform\" == \"macOS\" \]\]; then/,/^    elif/p' scripts/install/plan.sh); grep -Fq 'if [[ \"\$YBW_INSTALL_PLAN_MAC_APPS\" == true ]]' <<< \"\$block\" && grep -Fq 'ybw::command::require brew' <<< \"\$block\""
check "installer selects Home Manager Fish as the preferred macOS login shell" "grep -Fq '\$HOME/.local/state/nix/profiles/home-manager/home-path/bin/fish' scripts/install/platform.sh && grep -Fq 'sudo chsh -s \"\$fish_path\" \"\$USER\"' scripts/install/platform.sh && ! grep -Fq 'brew --prefix' scripts/install/platform.sh"
check "installer selects Linux Fish as login shell" "grep -Fq 'fish_path=\"\$(ybw::linux::find_fish)\"' scripts/install/packages.sh && grep -Fq 'ybw::linux::configure_fish_login_shell \"\$fish_path\"' scripts/install/packages.sh && ! grep -Fq 'configure_fish_login_shell \"\$(command -v fish)\"' scripts/install/platform.sh"
