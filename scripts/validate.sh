...
check "Home Manager installs delta through Git integration" "grep -Fq 'programs.delta.enable = true;' .config/home-manager/apps/git.nix && grep -Fq 'programs.delta.enableGitIntegration = true;' .config/home-manager/apps/git.nix && ! grep -Fq 'pkgs.delta' .config/home-manager/home.nix"
